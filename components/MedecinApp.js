...
export default function MedecinApp({ user, onLogout }) {
  ...
  function getQuota() {
    let indW = 0, indWE = 0
    Object.entries(dispoMap).forEach(([d, v]) => {
      if (v === 'indispo') indW++
  ...
  function toggleDay(ds) {
    ...
    if (cur === '') { next = 'dispo' }
      ...
      if (we) {
        if (indWE >= MAX_INDISPO_WE) { alert(`Maximum ${MAX_INDISPO_WE} jours de week-end indisponibles par mois atteint.`); return }
        next = 'we-indispo'
      } else {
  ...
  async function saveDispo() {
    ...
    const rows = Object.entries(dispoMap).map(([date, statut]) => ({
      medecin_id: user.id, date, statut
    }))
    ...
    <div>
      ...
      <div className="container">
        <div className="dispo-header">
          ...
          <div className="dispo-sub">Encodez vos disponibilités pour chaque mois. L'administrateur en tiendra compte lors de la génération du planning.</div>
        </div>
        ...
        <div className="hint-box">
          ...
          Disponibilités : illimitées · Indisponible semaine : max {MAX_INDISPO_WEEK} jours · Indisponible week-end : max {MAX_INDISPO_WE} jours
        </div>
        ...
        <div className={`quota-box ${qCls}`}>
          Jours indisponibles semaine : <strong>{indW} / {MAX_INDISPO_WEEK}</strong> &nbsp;·&nbsp;
          Jours indisponibles week-end : <strong>{indWE} / {MAX_INDISPO_WE}</strong><br/>
          Reste : <strong>{Math.max(0,wLeft)} jour(s) semaine</strong> · <strong>{Math.max(0,weLeft)} jour(s) week-end</strong>
        </div>
        ...
        <div className="des-legend">
          ...
          <span className="des-leg"><span className="ldot" style={{background:'#FAC775',borderRadius:2}}></span>Indisponible (week-end)</span>
        </div>
              ...
              {Array.from({length: Math.ceil(total/7)}, (_,ri) => (
                <tr key={ri}>
                  {Array.from({length:7}, (_,ci) => {
                    ...
                    if (!cell) return <td key={ci}><div className="dc empty-d">{cell}</div></td>
                    const ds = dateStr(y, m, cell)
                    const we = ci >= 5
                    const v = dispoMap[ds] || ''
                    const cls = v === 'dispo' ? 'dispo' : v === 'indispo' ? 'indispo' : v === 'we-indispo' ? 'we-indispo' : we ? 'is-we' : ''
                    return (
                      <td key={ci}>
                        <div className={`dc ${cls}`} onClick={() => toggleDay(ds)}>{cell}</div>
                      </td>
                    )
                  })}
                </tr>
